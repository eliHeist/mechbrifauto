...
<section class="content-grid bg-dark py-28 md:py-40">
	<div class="">
		<div
			class="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-16">
			<div>
				<!-- <p
					class="font-['Inter'] text-xs font-600 tracking-[0.3em] text-accent uppercase mb-4">
					What We Do
				</p> -->
				<h2
		...
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-white/10">
			{#each services as service, i}
				<div class="transition-all duration-700 bg-dark p-10 hover:bg-[#1A1A1A] group">
					<div
						class="font-['Inter'] text-xs tracking-[0.3em] text-accent uppercase mb-6">
						{service.label}
					</div>
					<h3
						class="font-['Barlow_Condensed'] font-800 text-white uppercase text-3xl leading-tight mb-4 group-hover:text-accent transition-colors">
						{service.title}
					</h3>
					...
					<div
						class="mt-8 w-8 h-px bg-accent group-hover:w-16 transition-all duration-300">
					</div>
				</div>
			{/each}
		</div>
